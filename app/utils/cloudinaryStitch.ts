...
type UploadVideoResp = { public_id: string };
type UploadImageResp = { secure_url: string };
...
async function uploadClip(localUri: string): Promise<string> {
  ...
  const json = (await res.json()) as UploadVideoResp;
...
export async function uploadImage(localUri: string): Promise<string> {
  ...
  const form = new FormData();
  form.append("upload_preset", PRESET);
  form.append("file", { uri: localUri, name: "face.png", type: "image/png" } as any);
  ...
  const json = (await res.json()) as UploadImageResp;
  return json.secure_url; // https://res.cloudinary.com/.../image/upload/...
}
